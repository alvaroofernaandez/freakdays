...
describe('useProfile', () => {
  ...
  it('sube avatar con signed URL y confirma en backend', async () => {
    ...
    expect(mockApi.post).toHaveBeenNthCalledWith(1, '/v1/profile/me/avatar/upload-url', {
    ...
    expect(mockApi.post).toHaveBeenNthCalledWith(2, '/v1/profile/me/avatar/confirm', {
      key: 'profiles/clerk_123/avatars/test.jpg',
    });
  ...
  it('si upload banner API falla retorna null sin fallback legacy', async () => {
    ...
    expect(mockFetch).not.toHaveBeenCalledWith('/api/profile/user-1', expect.anything());
  });
});
