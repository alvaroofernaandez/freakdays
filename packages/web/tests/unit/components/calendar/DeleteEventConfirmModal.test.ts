...
import DeleteEventConfirmModal from '../../../../app/components/calendar/DeleteEventConfirmModal.vue';
import type { Release } from '../../../../app/composables/useCalendar';

describe('DeleteEventConfirmModal.vue', () => {
  ...
  function mountComponent(
    ...
    return mount(DeleteEventConfirmModal, {
      props,
      global: {
        stubs: globalStubs,
      },
  ...
  describe('rendering', () => {
    ...
    it('should display delete confirmation title', () => {
      const wrapper = mountComponent();
      expect(wrapper.text()).toContain('Eliminar Evento');
    });

    it('should display release title in confirmation', () => {
      const wrapper = mountComponent();
  ...
  describe('events', () => {
    ...
    it('should emit confirm with release id when confirm clicked', async () => {
      const wrapper = mountComponent();
      const confirmButton = wrapper.findAll('button').find((b) => b.text().includes('Eliminar'));
      await confirmButton?.trigger('click');
      expect(wrapper.emitted('confirm')).toBeTruthy();
      expect(wrapper.emitted('confirm')?.[0]).toEqual(['event-1']);
    });
  });
