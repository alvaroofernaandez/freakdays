...
describe('TierListEditor.vue', () => {
    ...
    content: {
      tiers: [
        { id: 's', name: 'S', color: '#FF7F7F', items: [] },
        {
          ...
          name: 'A',
          color: '#FFBF7F',
          items: [{ id: 'item-1', content: 'Test Item', type: 'text' as const }],
        },
      ],
  ...
  describe('rendering', () => {
    ...
    it('should display list name in input', async () => {
      const wrapper = mountComponent();
      await nextTick();
      const nameInput = wrapper.find('input[aria-label="Nombre de la tier list"]');
    ...
    it('should create default tiers when content is empty', async () => {
      const wrapper = mountComponent();
      await nextTick();
      const tierElements = wrapper.findAll('[role="group"][aria-label^="Tier "]');
  ...
  describe('removeTier', () => {
    it('should render delete button for each tier', async () => {
      ...
      const deleteButtons = wrapper.findAll('button[aria-label^="Eliminar tier"]');
  ...
  describe('save button', () => {
    it('should render save button', async () => {
      ...
      const saveButton = wrapper.find('button[aria-label="Guardar cambios en la tier list"]');
  ...
  describe('accessibility', () => {
    it('should have proper aria labels', async () => {
      ...
      expect(wrapper.find('[aria-label="Editor de Tier List"]').exists()).toBe(true);
      expect(wrapper.find('[aria-label="Tiers de clasificación"]').exists()).toBe(true);
    });
  });
});
