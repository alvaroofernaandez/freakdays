...
const progressPercentage = computed(() => (selectedCount.value / totalModules.value) * 100);
...
<template>
  <div class="min-h-screen-safe flex flex-col relative z-10">
    <div
      class="flex-1 flex flex-col max-w-lg mx-auto w-full px-4 sm:px-6 py-6 sm:py-10 relative z-10"
    >
      ...
      <div class="flex-1 space-y-3 overflow-y-auto overflow-x-visible pb-4 -mx-4 px-4" role="main">
        ...
        >
          <button
            ...
            @keydown="handleKeyDown($event, module.id)"
          >
            <Card
              :class="[
      ...
      <footer class="pt-6 space-y-4 border-t border-border mt-4 relative z-10" role="contentinfo">
        <div class="space-y-3">
          <div class="flex items-center justify-between text-sm">
            <span class="text-muted-foreground font-medium"> Progreso de selección </span>
            <Badge
          ...
          <div class="relative h-2 bg-muted rounded-full overflow-hidden">
            <div
              class="absolute inset-y-0 left-0 bg-primary transition-all duration-500 ease-out rounded-full"
        ...
        <Button
          size="lg"
          class="w-full h-14 text-base font-semibold relative overflow-hidden group"
          :disabled="!canContinue"
          :aria-busy="saving"
          :aria-label="saving ? 'Guardando configuración...' : 'Comenzar aventura'"
