...
<template>
  <Dialog :open="open" @update:open="(val) => !val && emit('close')">
    <DialogContent
      class="max-w-[95vw] w-full h-[85vh] flex flex-col p-0 gap-0 overflow-hidden sm:max-w-6xl"
    >
      <DialogHeader
        class="px-6 py-4 border-b shrink-0 flex flex-row items-center justify-between space-y-0"
      >
        <div class="flex flex-col gap-1">
          <DialogTitle v-if="party">{{ party.name }}</DialogTitle>
          <DialogDescription v-if="party">
            {{ party.members.length }}/{{ party.maxMembers }} miembros • Código:
            <span class="font-mono bg-muted px-1 rounded">{{ party.inviteCode }}</span>
          </DialogDescription>
        </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <Card v-for="member in party.members" :key="member.id">
                  <CardHeader class="flex flex-row items-center gap-4 space-y-0">
                    <div
                      class="h-10 w-10 rounded-full bg-muted flex items-center justify-center overflow-hidden"
                    >
                      <img
  ...
  <PartyListDetailModalInternal
    ...
    :list="selectedList"
    :party-id="party?.id || ''"
    @update:open="(val) => !val && handleBackToLists()"
    @close="handleBackToLists"
  />
