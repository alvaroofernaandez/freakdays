  ...
  <ClientOnly>
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="open && release"
          class="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6 bg-background/95 backdrop-blur-sm overflow-y-auto"
          style="pointer-events: auto;" @click.self="emit('close')" @keydown.esc="emit('close')" role="dialog"
          aria-modal="true" aria-labelledby="delete-confirm-title" aria-describedby="delete-confirm-description">
          <Card class="w-full max-w-md shadow-xl border-2 border-destructive/20 my-auto" @click.stop role="document">
            <CardHeader class="p-4 sm:p-6 pb-3 sm:pb-4">
              ...
              <CardDescription class="text-sm sm:text-base mt-2" id="delete-confirm-description">
                ¿Estás seguro de que quieres eliminar <strong>"{{ release.title }}"</strong>? Esta acción no se puede
                deshacer.
              </CardDescription>
            </CardHeader>
            <CardContent class="flex flex-col sm:flex-row gap-2 p-4 sm:p-6 pt-0">
              <Button variant="outline" class="flex-1 min-h-[44px] touch-manipulation" @click="emit('close')"
                :disabled="isSubmitting" aria-label="Cancelar eliminación">
                Cancelar
              </Button>
              <Button variant="destructive" class="flex-1 min-h-[44px] touch-manipulation cursor-pointer"
                @click="emit('confirm', release.id)" :disabled="isSubmitting"
