...
<template>
  <div class="space-y-3 p-4 sm:p-5 border rounded-xl bg-card/50">
    ...
    <div
      ...
      class="text-center py-6 sm:py-8 border-2 border-dashed rounded-lg bg-muted/30"
    >
    ...
    <div v-else class="space-y-2">
      ...
      >
        <div
          class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-primary/10 shrink-0"
        >
            ...
            <div class="relative">
              <Input
                ...
                :class="{
                  'opacity-50':
                    updatingSets &&
                    typeof updatingSets.has === 'function' &&
                    updatingSets.has(set.id),
                ...
                "
                @update:model-value="
                  (val) => handleSetUpdate(exercise.id, set.id, 'reps', val as string)
                "
                @blur="handleSetBlur(exercise.id, set.id)"
              />
              <div
                v-else
                class="h-10 sm:h-11 flex items-center justify-center text-base font-medium"
              >
                {{ set.reps ?? '-' }}
              </div>
              <div
          ...
          <div class="space-y-1">
            ...
            <div class="relative">
              <Input
                ...
                "
                @update:model-value="
                  (val) => handleSetUpdate(exercise.id, set.id, 'weightKg', val as string)
                "
                @blur="handleSetBlur(exercise.id, set.id)"
              />
              <div
