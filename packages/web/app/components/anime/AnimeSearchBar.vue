...
<template>
  <div
    class="relative w-full bg-background"
    style="
      position: relative;
      z-index: 10;
  ...
  >
    <div class="relative flex items-center w-full" style="pointer-events: auto !important">
      <Search
        v-if="!searching"
        class="absolute left-3 h-5 w-5 text-muted-foreground pointer-events-none z-10"
      />
      <Loader2
        v-else
        class="absolute left-3 h-5 w-5 text-primary animate-spin pointer-events-none z-10"
      />
      <Input
      ...
      <Button
        ...
        variant="ghost"
        size="icon"
        class="absolute right-1 h-8 w-8 hover:bg-muted z-10"
