...
<template>
  <ClientOnly>
    <div
      v-if="items.length > 0"
      class="flex items-center gap-0.5 overflow-x-auto scrollbar-hide"
      style="position: relative; z-index: 10000"
    >
      <div class="w-px h-4 bg-border/50 mx-2 shrink-0" />
      <NuxtLink
        v-for="item in items"
        :key="item.to"
        :to="item.to"
      ...
      >
        <component
          ...
          aria-hidden="true"
        />
        <span class="font-pixel text-[9px] uppercase hidden lg:inline">{{ item.label }}</span>
      </NuxtLink>
    </div>
  </ClientOnly>
</template>
