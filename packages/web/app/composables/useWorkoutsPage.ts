...
export function useWorkoutsPage() {
  ...
  async function startWorkout(workoutForm: {
    name: string;
    description: string;
    workout_date: string;
  }) {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (updates.reps !== undefined) {
          updatePayload.reps =
            updates.reps === null || updates.reps === '' ? null : Number(updates.reps);
        }
        if (updates.weight_kg !== undefined) {
          updatePayload.weight_kg =
            updates.weight_kg === null || updates.weight_kg === ''
              ? null
              : Number(updates.weight_kg);
        }
  ...
  async function saveSet(
    ...
    try {
      ...
      if (updates.weight_kg !== undefined) {
        updatePayload.weight_kg =
          updates.weight_kg === null || updates.weight_kg === '' ? null : Number(updates.weight_kg);
      }
  ...
  watch(
    () => currentWorkout.value?.startedAt,
    () => {
  ...
  async function initialize() {
    await Promise.all([reloadData(), checkInProgressWorkout()]);
  }
